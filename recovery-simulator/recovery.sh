...
if ! "${ENV_RESETTED:-false}"; then
  ...
  if test "${COVERAGE:-false}" = 'false'; then
    ...
    exec -- env -i -- ENV_RESETTED=true THIS_SCRIPT="${THIS_SCRIPT:?}" OUR_TEMP_DIR="${OUR_TEMP_DIR:?}" CI="${CI:-}" APP_NAME="${APP_NAME:-}" PATH="${PATH:?}" COVERAGE='true' PS4="${PS4:?}" PARALLEL_TEST_GROUPS='1' TEST_ENV_NUMBER="${RANDOM:?}" bashcov --command-name "Recovery simulator (after reset) - ${RANDOM:?}" -- "${THIS_SCRIPT:?}" "${@}" || fail_with_msg 'failed: exec'
  fi
  exit 127
fi
unset ENV_RESETTED
...
if test "${COVERAGE:-false}" != 'false'; then
  rm -rf -- "${OUR_TEMP_DIR:?}" || true
  OUR_TEMP_DIR="${THIS_SCRIPT_DIR:?}/ANDR-RECOV"
fi
