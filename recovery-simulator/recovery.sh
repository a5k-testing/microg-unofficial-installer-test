...
if ! "${ENV_RESETTED:-false}"; then
  ...
  if test "${COVERAGE:-false}" = 'false'; then
    ...
    exec -- env -i -- ENV_RESETTED=true THIS_SCRIPT="${THIS_SCRIPT:?}" OUR_TEMP_DIR="${OUR_TEMP_DIR:?}" CI="${CI:-}" APP_NAME="${APP_NAME:-}" PATH="${PATH:?}" COVERAGE='true' PARALLEL_TEST_GROUPS='1' bashcov --command-name "Recovery simulator (after reset) - ${RANDOM:?}" -- "${THIS_SCRIPT:?}" "${@}" || fail_with_msg 'failed: exec'
  fi
  exit 127
fi
unset ENV_RESETTED
